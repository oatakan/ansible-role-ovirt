...
DOCUMENTATION = """
    ...
    options:
      ...
      return_index:
        default: False
        description:
          - Returns enumerated lists. See Example section for more information.
          - If set to V(True), the lookup plugin will return enumerated items where item.*.0 is index, item.*.1 is data
          - If set to V(False), the plugin will return regular lists
"""
...
- name: show var structure as it is needed for example to make sense
  ...
  tasks:
    ...
    - name: list groups with index value
      ansible.builtin.debug: var=item.1
      loop: "{{ q('ansible.builtin.subelements', users, 'groups', {'return_index': True}) }}"
"""
...
FLAGS = ('skip_missing','return_index',)
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables, **kwargs):
        ...
        return_index = boolean(flags.get('return_index', False), strict=False)

        # build_items
        ret = []
        for index0, item0 in enumerate(elementlist):
            ...
            ret.extend(
                ([index0, item0], [index1, item1]) if return_index else (item0, item1)
                for index1, item1 in enumerate(sublist)
            )
